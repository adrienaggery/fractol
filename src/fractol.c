...
/*   Updated: 2014/12/30 19:55:31 by aaggery          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_init(t_fractol *fl)
{
	...
	if (ft_strequ("mandelbrot", argv[1]))
		...
		mlx_expose_hook(fl->win, draw_julia, fl);
	else if (ft_strequ("X", argv[1]))
		mlx_expose_hook(fl->win, draw_X, fl);
	fl->buffer.img = mlx_new_image(fdf->mlx, WIN_WIDTH, WIN_HEIGHT);
